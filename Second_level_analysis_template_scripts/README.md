...
| `LaBGAScore_atlas_rois_from_atlas.m`           | referenced by`roi_names`/`roi_modelname`/`roi_set_name` options in `a2_set_default_options.m` | Generates the per-ROI atlas objects `prep_3a` extracts ROI averages from. **Must be run first**, from the superdataset root: `prep_3a` loads `<maskdir>/<roi_modelname>_rois_<roi_set_name>.mat` and does not create it. `roi_modelname` is only a filename prefix — the file is loaded from the maskdir of the model being run, so reusing a set across models means copying the `.mat`. |
...
| `tfce_fwe_from_null.m`                        | `c2a_second_level_regression.m`                                                                    | Max-statistic FWE p-values from a saved TFCE permutation null, so runs saved before FWE existed can be corrected without re-permuting. |
| `LaBGAScore_region_table.m`                   | `c2a_second_level_regression.m`                                                                    | Region table vendored from CanlabCore's `@region/table`, with the clipping of large values removed. Upstream's `get_signed_max` ends with `maxZ = norminv(1 - 1E-12)` (= 7.0345) and clips **every** value above it, not only the infinities its comment describes, so any peak above 7.0345 is silently flattened. As of c2a v8.5 all five table branches use this copy. Before that only the TFCE branches did — the assumption being that t and BF maps stay under the ceiling, which they do not: model_2i's Bayes factor map reaches 2·ln(BF) = 107 and its t-map |t| = 14.8. Peak column only; no inference was affected. |
| `LaBGAScore_region_table_safe.m`              | `c2a_second_level_regression.m`                                                                    | Wrapper guaranteeing three outputs, so a contrast with nothing displayable does not abort the whole report. |
