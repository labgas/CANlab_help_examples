function [dist_from_hyperplane, Y, svm_dist_pos_neg, svm_dist_pos_neg_matrix] = plugin_oofmridataobj_svm_contrasts_get_results_per_subject(DAT, svm_stats_results, DATA_OBJ)
...
for c = 1:kc
    ...
    % reformat stats structure saved by prep_3c script with oofmridataobj
    % option to match what is produced by predict function
    
    % NOTE: may break when folds are not equal size, but not sure @lukasvo
    % There must be an easier way to do this with indices, but it works for
    % now ;)
    
    dist_from_hyperplane_temp = zeros(size(svm_stats_results{c}.cvGS.fold_lbls,1),1);
    yfit_temp = zeros(size(svm_stats_results{c}.cvGS.fold_lbls,1),1);
    yfit_null_temp = zeros(size(svm_stats_results{c}.cvGS.fold_lbls,1),1);
    Y_temp = zeros(size(svm_stats_results{c}.cvGS.fold_lbls,1),1);
    teIdx_temp = cell(1,size(svm_stats_results{c}.cvGS.foldEstimator,1));
    
    for f = 1:size(svm_stats_results{c}.cvGS.yfit,1)
        dist_from_hyperplane_fold = svm_stats_results{c}.cvGS.yfit_raw{f};
        yfit_fold = svm_stats_results{c}.cvGS.yfit{f};
        yfit_null_fold = svm_stats_results{c}.cvGS.yfit_null{1,f};
        Y_fold = svm_stats_results{c}.cvGS.Y{1,f};
        if f == 1
            dist_from_hyperplane_temp((1:(size(dist_from_hyperplane_fold,1)/2)),1) = dist_from_hyperplane_fold((1:(size(dist_from_hyperplane_fold,1)/2)),1);
            dist_from_hyperplane_temp((1+(size(dist_from_hyperplane_temp,1)/2):(size(dist_from_hyperplane_temp,1)/2)+(size(dist_from_hyperplane_fold,1)/2)),1) = dist_from_hyperplane_fold(((size(dist_from_hyperplane_fold,1)/2)+1:end),1);
            yfit_temp((1:(size(yfit_fold,1)/2)),1) = yfit_fold((1:(size(yfit_fold,1)/2)),1);
            yfit_temp((1+(size(yfit_temp,1)/2):(size(yfit_temp,1)/2)+(size(yfit_fold,1)/2)),1) = yfit_fold(((size(yfit_fold,1)/2)+1:end),1);
            yfit_null_temp((1:(size(yfit_null_fold,1)/2)),1) = yfit_null_fold((1:(size(yfit_null_fold,1)/2)),1);
            yfit_null_temp((1+(size(yfit_null_temp,1)/2):(size(yfit_null_temp,1)/2)+(size(yfit_null_fold,1)/2)),1) = yfit_null_fold(((size(yfit_null_fold,1)/2)+1:end),1);
            Y_temp((1:(size(Y_fold,1)/2)),1) = Y_fold((1:(size(Y_fold,1)/2)),1);
            Y_temp((1+(size(Y_temp,1)/2):(size(Y_temp,1)/2)+(size(Y_fold,1)/2)),1) = Y_fold(((size(Y_fold,1)/2)+1:end),1);
        else
            dist_from_hyperplane_temp((1+(size(dist_from_hyperplane_fold,1)/2)*(f-1):((size(dist_from_hyperplane_fold,1)/2)*(f-1)+size(dist_from_hyperplane_fold,1)/2)),1) = dist_from_hyperplane_fold((1:(size(dist_from_hyperplane_fold,1)/2)),1);
            dist_from_hyperplane_temp((1+(size(dist_from_hyperplane_temp,1)/2)+(size(dist_from_hyperplane_fold,1)/2)*(f-1):((size(dist_from_hyperplane_temp,1)/2)+((size(dist_from_hyperplane_fold,1)/2)*(f-1))+(size(dist_from_hyperplane_fold,1)/2))),1) = dist_from_hyperplane_fold(((size(dist_from_hyperplane_fold,1)/2)+1:end),1);
            yfit_temp((1+(size(yfit_fold,1)/2)*(f-1):((size(yfit_fold,1)/2)*(f-1)+size(yfit_fold,1)/2)),1) = yfit_fold((1:(size(yfit_fold,1)/2)),1);
            yfit_temp((1+(size(yfit_temp,1)/2)+(size(yfit_fold,1)/2)*(f-1):((size(yfit_temp,1)/2)+((size(yfit_fold,1)/2)*(f-1))+(size(yfit_fold,1)/2))),1) = yfit_fold(((size(yfit_fold,1)/2)+1:end),1);
            yfit_null_temp((1+(size(yfit_null_fold,1)/2)*(f-1):((size(yfit_null_fold,1)/2)*(f-1)+size(yfit_null_fold,1)/2)),1) = yfit_null_fold((1:(size(yfit_null_fold,1)/2)),1);
            yfit_null_temp((1+(size(yfit_null_temp,1)/2)+(size(yfit_null_fold,1)/2)*(f-1):((size(yfit_null_temp,1)/2)+((size(yfit_null_fold,1)/2)*(f-1))+(size(yfit_null_fold,1)/2))),1) = yfit_null_fold(((size(yfit_null_fold,1)/2)+1:end),1);
            Y_temp((1+(size(Y_fold,1)/2)*(f-1):((size(Y_fold,1)/2)*(f-1)+size(Y_fold,1)/2)),1) = Y_fold((1:(size(Y_fold,1)/2)),1);
            Y_temp((1+(size(Y_temp,1)/2)+(size(Y_fold,1)/2)*(f-1):((size(Y_temp,1)/2)+((size(Y_fold,1)/2)*(f-1))+(size(Y_fold,1)/2))),1) = Y_fold(((size(Y_fold,1)/2)+1:end),1);
        end
        teIdx_temp{1,f} = logical(svm_stats_results{c}.cvGS.fold_lbls == f);
    end
    
    
    
    svm_stats_results{c}.dist_from_hyperplane_xval = dist_from_hyperplane_temp;
    svm_stats_results{c}.yfit = yfit_temp;
    svm_stats_results{c}.yfit_null = yfit_null_temp;
    svm_stats_results{c}.Y = Y_temp;
    svm_stats_results{c}.teIdx = teIdx_temp;
    
    stats = svm_stats_results{c};
    
    clear dist_from_hyperplane_fold dist_from_hyperplane_temp yfit_temp yfit_fold yfit_null_temp yfit_null_fold Y_temp Y_fold f
...
for c = 1:kc
    ...
    for c2 = 1:kc
        ...
        for f = 1:length(fold_testset)
            ...
            w = stats.cvGS.foldEstimator{f}.estimator.estimator.B;     % weights for this fold, cross-validated (from subjects not used in test)
            intc = stats.cvGS.foldEstimator{f}.estimator.estimator.offset;  % intercept for this fold % MAY BE WRONG
